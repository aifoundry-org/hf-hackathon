...
- **L4 - `tensor_load` source stride is 64-byte-granular.** So the A operand is always one row per
  SCP line; a 16-wide int8 row uses only 16 of 64 bytes -> 4x eviction waste vs a dense quartet pack.
...
- **L6 - We are NOT bandwidth-bound; the kernel scales.** The 1/2/4/8-hart sweep gives 6.98x
...
- **L9 - "Near the floor" was a false generalization; name the axis each experiment tested.** Two
  ...
  *neither touched per-tile dispatch overhead*, the axis B8 then won on. Two compounding mistakes:
  ...
  hart0 IPC (2.64) as "waits don't matter" when the 9x per-tile blocking `WAIT`s were a fixed serial
...
- **L10 - Don't judge a rolled-loop body change by static code size.** The pack word-copy (P1) barely
  ...
  **-18 %** wall. The loops stayed rolled, so static size hid a 4x *dynamic* reduction (word vs byte
...
|---:|---:|---:|---:|---:|
| 1 | 75.32 ms | 0.392 | 1.00x | 100 % |
| 2 | 39.48 ms | 0.747 | 1.91x | 95.4 % |
| 4 | 19.98 ms | 1.476 | 3.77x | 94.2 % |
| 8 | 10.78 ms | 2.735 | 6.98x | 87.3 % |
...
- **Hypothesis:** the kernel is instruction-bound (L1); a transposed GEMM makes the output store a
  ...
  cheap contiguous copy (deletes the 9x256 quartet weave) -> fewer instructions -> faster.
...
- **Result:** correct (`max_abs=0`), retired-inst **-11.7 %** - but wall **+8.4 %** (10.73 -> 11.64 ms),
  IPC **2.64 -> 2.14**, DDR traffic **+3.2x**, L2 +60 %.
- **Lesson (-> L2, L3, L4):** the instruction win was real but the transpose moved the 16-line operand
  from static weights (evicted once/layer) to per-tile activations (evicted every tap, 4x the bytes),
...
- **Result:** correct (`max_abs=0`), wall **-4.34 %** - board A/B (plain, interleaved, 5x): baseline
  median 10.43 ms vs B8 **9.98 ms**, **non-overlapping** (baseline min 10.39 > B8 max 10.02), spread
  1.28 %. Bonus: A-load traffic ~3x lower (weights loaded once/group, not once/tap). **Committed.**
...
- **Result:** correct (`max_abs=0`), **-2.68 %** on B8 - A/B (plain, interleaved, 5x): B8 median
...
- **Lesson (-> L8):** same axis as B8, one notch further. Now spent: the remaining per-tile waits are
  the *sequential* `LOAD_WAIT`/`FMA_WAIT` (3 groups), not batchable - 3 groups' A+B (3x12+16 lines)
...
- **Result:** correct (`max_abs=0`), **-18.34 %** on B8+B1 - A/B (plain, interleaved, 5x): B8+B1 median
...
- **Lesson (-> L10):** the pack copy-cost was the biggest single lever of the run and it was
  layout-neutral. The static op-count "barely moved" - the win was purely the 4x *dynamic* reduction
  from rolled loops, only visible on the board.

## Status - B8+B1+P1 shipped (~7.90 ms, -24.24 % end-to-end, direct 8x A/B)

New best: **~7.90 ms** (was ~10.43 ms) - **direct baseline-vs-full A/B (8x): -24.24 %**, non-overlapping
(baseline min 10.39 > full max 7.93), `max_abs=0`, **~2.9x the leaderboard leader** (was ~3.8x).
Four wins stacked, on two distinct productive axes:

| step | change | wall | delta  |
|---|---|---:|---:|
| baseline | - | 10.46 ms | - |
...
writes). A/B (5x): -0.76 %, **inside** the 2.06 % spread -> not a win. The compiler had already
...
|---|---|---|---|
| **B6** depth-first fusion | kill intermediate DRAM round-trips + 2 barriers | **Predicted REGRESSION** | both targets are non-bottlenecks (not bandwidth-bound; B3 showed barrier restructuring regressed) *and* the ~1.9x redundant halo tiles = ~1.9x redundant `pack_B` - inflating the one sink P1 proved dominant. Trades non-bottleneck saving for bottleneck cost. |
...
| **B7** weight/activation residency + coop load | operand-reload traffic | **Predicted LOW** | not bandwidth-bound, so traffic cuts don't move wall (B3); B8 already cut A-load ~3x. Coop-load might shave a few *load instructions* - marginal at best. |
...
| Winograd / separable / FFT / INT4 / block-sparse | fewer MACs | **Rejected (unchanged)** | we are not MAC-bound; all cut FLOPs, not the scalar/overhead critical path (original research sec D still holds, now with the measurement to back it). |
...
to a 1x1 GEMM, the structural fix for the pack) or **policy-gated >8-hart / multi-shire** parallelism
