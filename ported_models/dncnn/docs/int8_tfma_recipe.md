...
- **Result:** bit-exact (`max_abs=0`) vs the int8 reference at 1 hart (sys-emu) and 8
  harts (board); ~0.0107 s kernel wait on the board (≈2.9× the FP32 winner's 0.0314 s).

## 1. Network and precision split

The DnCNN graph here is `conv_first -> 3 hidden convs -> conv_final`, all 3×3, with
...
from (int8 TFMA is the single biggest lever — see `docs/optimizations.md §1.1`).

## 2. int8 hidden layer on the tensor engine

Each hidden output tile is 16 OC × 16 spatial. Per tile (`conv_tile`):

1. **Pack activations** (`pack_b_tile`): gather the 3×3 window into the
...
2. **9 taps** (`fma_tap` → `tensor_fma`, opcode 3 = int8): one dispatch per `(ky,kx)`
   tap accumulates 16 OC × 16 IC × 16 spatial into the int32 TENC accumulator.
...
3. **Requant** (`requant_u8` → `tensor_quant`): fused int32 → uint8 in the float
   registers — dequant × per-tensor scale, round-nearest-even, ReLU + clamp[0,255],
...
1. One int8 `tensor_fma` tap (16×16×16) → verify one MAC tile vs numpy.
...
- **Correctness:** compare the output image at `OUTPUT_OFFSET` (0x10000, 64×64 uint8)
...
- **CI gate:** read the 16×`uint32` summary at `SUMMARY_OFFSET` (0x1000) and confirm
