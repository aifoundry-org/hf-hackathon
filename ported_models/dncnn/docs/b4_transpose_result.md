...
saving was overwhelmed by a 1.5–3× increase in memory traffic, so `kernel_wait_s` went *up*.
The experiment is a clean negative result that reframes the optimization plan (see §4).
...
- **Pack activations as the plain row-major A operand** (16 contiguous IC bytes per spatial) —
  replacing the 9×256 scalar quartet-interleave with a cheap contiguous copy (the pack_B win).
...
| Metric | Baseline (OC-major) | Transposed | Δ |
|---|---:|---:|---:|
...
| **DDR reads / writes** | 37,500 / 36,380 | 119,797 / 115,895 | **+3.19× / +3.19×** |
...
1. **Sparse A-operand eviction (dominant).** Activations moved from the **dense** 4-line quartet
   ...
   the run (768 tiles × 9 taps × +768 B). This is the bulk of the L2-write / DDR increase.
...
- **The plan needs a memory-traffic axis, not just instruction count.** The "instruction-bound"
  ...
  - **B1 (batch the 9 per-tap evicts into one)** now matters for a second reason — fewer, larger
    ...
    own it will not undo a 4× byte increase. Measure, don't assume.
  ...
  - A pure output-store win (deleting the scatter) is **not worth 4× activation eviction** — only
...
described in §1; the deltas that killed it are §2–§3. If ever revisited, the board-run recipe is
