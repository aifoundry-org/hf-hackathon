/*
 * Real Whisper LayerNormalization audit kernel.
 *
 * Each hart owns whole rows, so each row's mean/variance reduction is local to
 * one hart. This is the right shape for Whisper LayerNorm tensors where the
 * normalized dimension is 384 and the outer dimensions are rows.
 */
...
#endif
#ifndef IN_OFFSET
#define IN_OFFSET         0x10000u
#endif
#ifndef WEIGHT_OFFSET
#define WEIGHT_OFFSET     0x200000u
#endif
#ifndef BIAS_OFFSET
#define BIAS_OFFSET       0x210000u
#endif
#ifndef REF_OFFSET
#define REF_OFFSET        0x220000u
#endif
...
int main(uintptr_t arg_area)
{
	...
	const float *const in = (const float *)(base + IN_OFFSET);
	const float *const weight = (const float *)(base + WEIGHT_OFFSET);
	const float *const bias = (const float *)(base + BIAS_OFFSET);
	...
	if (hart_id == 0u) {
		FENCE;
		evict((void *)in, ROWS * LN_DIM * sizeof(float));
		evict((void *)weight, LN_DIM * sizeof(float));
		evict((void *)bias, LN_DIM * sizeof(float));
		evict((void *)ref, ROWS * LN_DIM * sizeof(float));
		WAIT_CACHEOPS;
	}
	bench_barrier();
	...
	for (uint32_t r = row0; r < row1; r++) {
		const float *const src = in + r * LN_DIM;
		float *const dst = out + r * LN_DIM;
		float sum = 0.0f;
		float sumsq = 0.0f;

		for (uint32_t c = 0u; c < LN_DIM; c++) {
			const float v = src[c];
		...
		for (uint32_t c = 0u; c < LN_DIM; c++) {
			dst[c] = ((src[c] - mean) * inv) * weight[c] + bias[c];
		}
	}

	FENCE;
	if (row1 > row0) {
		evict(out + row0 * LN_DIM,
		      (row1 - row0) * LN_DIM * sizeof(float));
		WAIT_CACHEOPS;
	}
	bench_barrier();
	...
	slot->checksum = hash_f32(out, row0 * LN_DIM, row1 * LN_DIM);
	slot->done = 1u;
	FENCE;
	...
	if (hart_id == 0u) {
		...
		for (uint32_t i = 0u; i < ROWS * LN_DIM; i++) {
