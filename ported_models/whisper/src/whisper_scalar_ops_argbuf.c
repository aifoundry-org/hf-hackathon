/*
 * Real Whisper scalar-node audit kernel.
 *
 * This is the scalar side of the native decoder bring-up: elementwise Add/Mul,
 * Div, Erf/GELU-shaped math, and row-wise Softmax on real ONNXRuntime tensors.
 * Inputs are supplied through the soc1sim arg buffer and the summary is written
 * back into the same buffer for host-side auditing.
 */
...
#define PARTIAL_OFFSET    0x2000u
#endif
#ifndef PARAM_OFFSET
#define PARAM_OFFSET      0x3000u
#endif
#ifndef OUT_OFFSET
#define OUT_OFFSET        0x4000u
#endif
#ifndef A_OFFSET
#define A_OFFSET          0x10000u
#endif
#ifndef B_OFFSET
#define B_OFFSET          0x40000u
#endif
#ifndef REF_OFFSET
#define REF_OFFSET        0x70000u
#endif
...
static inline float compute_elem(float a, float b)
{
#if OP_MODE == OP_ADD
	return a + b;
#elif OP_MODE == OP_MUL
...
static void run_elementwise(uint32_t hart_id, float *out, const float *a,
			    const float *b)
{
...
static void mark_phase(volatile struct audit_slot *slots, uint32_t hart_id,
		       uint32_t phase)
{
	volatile struct audit_slot *const slot =
		(volatile struct audit_slot *)((volatile uint8_t *)slots +
					      hart_id * SLOT_BYTES);
...
static void run_softmax(uint32_t hart_id, float *out, const float *a,
			volatile struct softmax_partial *partials,
			volatile float *params,
			volatile struct audit_slot *slots)
{
	(void)partials;
	(void)params;
	(void)slots;

	const uint32_t row0 = (ROWS * hart_id) / ACTIVE_HARTS;
	const uint32_t row1 = (ROWS * (hart_id + 1u)) / ACTIVE_HARTS;
	...
	for (uint32_t row = row0; row < row1; row++) {
		...
		for (uint32_t c = 0u; c < COLS; c++) {
			const float e = fast_expf_softmax(a[base + c] - maxv);
...
int main(uintptr_t arg_area)
{
	...
	const float *const a = (const float *)(base + A_OFFSET);
	const float *const b = (const float *)(base + B_OFFSET);
	const float *const ref = (const float *)(base + REF_OFFSET);
	volatile struct softmax_partial *const partials =
		(volatile struct softmax_partial *)(base + PARTIAL_OFFSET);
	volatile float *const params = (volatile float *)(base + PARAM_OFFSET);
	...
	if (hart_id == 0u) {
		FENCE;
		evict((void *)a, ELEM_COUNT * sizeof(float));
		evict((void *)b, ELEM_COUNT * sizeof(float));
...
#if OP_MODE == OP_SOFTMAX
	run_softmax(hart_id, out, a, partials, params, slots);
#else
	run_elementwise(hart_id, out, a, b);
#endif
	bench_barrier();
