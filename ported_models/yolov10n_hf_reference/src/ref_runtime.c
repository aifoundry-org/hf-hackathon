    ...
    float *output)
{
    ...
    for (n = 0; n < batches; ++n) {
        for (oc = 0; oc < output_channels; ++oc) {
            ...
            const uint32_t input_channel_base = group * channels_per_group;
            ...
            for (oh = 0; oh < output_h; ++oh) {
                for (ow = 0; ow < output_w; ++ow) {
                    ...
                    for (icg = 0; icg < channels_per_group; ++icg) {
                        const uint32_t ic = input_channel_base + icg;
                        for (ky = 0; ky < (uint32_t)node->kernel_h; ++ky) {
                            const int32_t ih =
                            ...
                            for (kx = 0; kx < (uint32_t)node->kernel_w; ++kx) {
                                const int32_t iw =
                                ...
                                weight_index =
                                    ((oc * channels_per_group + icg)
                                     * (uint32_t)node->kernel_h + ky)
                                    * (uint32_t)node->kernel_w + kx;
                                accumulator +=
                                    input[input_index] * weight[weight_index];
                            }
                        }
                    }
                    output[((n * output_channels + oc) * output_h + oh)
                           * output_w + ow] = accumulator;
                }
            }
        }
    }
    ...
    float *output)
{
    ...
    for (index = 0; index < output_desc->elements; ++index) {
    ...
    uint32_t subtract)
{
    ...
    for (index = 0u; index < output_desc->elements; ++index) {
        output[index] =
    ...
    float *output)
{
    ...
    for (n = 0u; n < input_desc->dims[0]; ++n) {
        for (channel = 0u; channel < input_desc->dims[1]; ++channel) {
            for (oh = 0u; oh < output_h; ++oh) {
    ...
    for (node_index = first_local_node;
        ...
        if (node->op == YR_OP_CONV) {
            ...
            status = yr_add_sub(
                ...
                (float *)out0_raw, node->op == YR_OP_SUB);
